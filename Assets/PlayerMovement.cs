...
public class PlayerMovement : MonoBehaviour
{
    ...
    IEnumerator DashRoutine()
    {
        ...
        Vector3 originalMoveDirection = lookingDirection;
        Quaternion dashRotation = Quaternion.LookRotation(originalMoveDirection);

        dashFX.Play();
        while (Time.time < startTime + dashTime)
        {
            characterController.Move(originalMoveDirection * dashSpeed * Time.deltaTime);
            yield return null;
        }
